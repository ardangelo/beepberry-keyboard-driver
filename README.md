...
- Keyboard Pre-requisites

  1. The Raspberry Pi must have kernel drivers installed on it.
    ```bash
      sudo apt install raspberrypi-kernel-headers
     ```
  2. The Pi must have its I2C Peripheral enabled. 

- Keyboard to Raspberry Pi Hardware connections.
```
...
```
 ...
 3. Check pre-requisites with the command `i2cdetect -y 1`. The Keyboard should be on the address `0x1F`. If you are using the [Keyboard FeatherWing](https://www.tindie.com/products/arturo182/keyboard-featherwing-qwerty-keyboard-26-lcd/), the Touch sensor on the screen will also show up on `0x0A`.
...
### Installation: As Newbie or Expert.
- Newbie Installation: **Make sure the pre-requisites mentioned above are fully met before running the command**
  ```bash
...
```bash
   ...
   GPIO9(Pi Pin 21/BCM GPIO 9) ---> LCD_DC on Keyboard FeatherWing.
...
## Customisation
 ...
 - For changing the Interrupt Pin on the Raspberry Pi, a few changes are needed.
   1. If following the newbie Installation, change the line `sudo sh -c "echo dtoverlay=i2c-bbqX0kbd,irq_pin=22 >> /boot/config.txt"` to `sudo sh -c "echo dtoverlay=i2c-bbqX0kbd,irq_pin=BCM_PIN >> /boot/config.txt"` where `BCM_PIN` is the BCM Pin on the Pi Header. By default, it's set to GPIO 22, or the Pin 15 of the Raspberry Pi. If using the Expert method, make the same changes in `preInstaller.sh` file.
...
## Known Issues
  ...
  - The `ESC` Key assigned above `BACKSPACE` Key hasn't functioned the best in `nano`
